---
name: source-check
description: Verifies citations in two passes, writing REFERENCES.md and SOURCE-CHECK.md. First, does each work exist as cited, against publisher or DOI records. Second, does each source actually say what the document claims, checked against primary text with page-level quotes. Failed claims first, load-bearing ones flagged, and an explicit count of what was and was not checked. Covers numeric figures too — emission factors, thresholds, GWP values — where the rule is a primary source plus retrieval date and vintage. Use before publishing anything that cites, when adding a published factor to code, whenever a claim rests on a summary rather than primary text, or on "kaynaklari dogrula", "atiflari kontrol et", "does this source actually say that". Not for finding new sources; use deep-research.
---
